...
class ScoreboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final headerBg = isDark ? const Color(0xFF131315).withOpacity(0.8) : Colors.white.withOpacity(0.8);
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          _buildSliverAppBar(context, isDark, headerBg, accentColor),
          SliverFillRemaining(
            hasScrollBody: false,
            child: _buildMainContent(context, ref, state, isDark, accentColor, cardBg, secondaryTextColor),
          ),
        ],
      ),
    );
  ...
  Widget _buildSliverAppBar(BuildContext context, bool isDark, Color bgColor, Color accentColor) {
    return SliverAppBar(
      pinned: true,
      backgroundColor: bgColor,
      elevation: 0,
      ...
      leading: IconButton(
        icon: Icon(Iconsax.arrow_left_copy, color: accentColor),
      ...
      title: Row(
        ...
        children: [
          Icon(Iconsax.flash_copy, color: accentColor, size: 24),
      ...
      actions: [
        IconButton(
          icon: Icon(Iconsax.notification_copy, color: accentColor, size: 20),
          onPressed: () {},
        ),
        const SizedBox(width: 8),
      ],
      ...
      children: [
        ...
        Expanded(
          child: Padding(
            ...
            child: Row(
              children: [
                Expanded(child: _buildPlayerColumn(ref, true, state.scoreA, 'Chen Long', isDark, accentColor, cardBg)),
                const SizedBox(width: 16),
                Expanded(child: _buildPlayerColumn(ref, false, state.scoreB, 'T. Ying', isDark, accentColor, cardBg)),
              ],
            ),
          ),
        ),
        const SizedBox(height: 24),
        _buildBottomControls(context, ref, isDark, accentColor, secondaryTextColor),
      ],
    );
  }

  Widget _buildTimerPill(bool isDark, Color accentColor, Color secondaryTextColor) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isDark ? const Color(0xFF2C2C2F) : Colors.black.withOpacity(0.05)),
        boxShadow: [
          BoxShadow(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(height: 16, width: 1, color: isDark ? const Color(0xFF2C2C2F) : Colors.black.withOpacity(0.1)),
          Row(
            children: [
              Text(
                'SET',
  ...
  Widget _buildPlayerColumn(
    ...
    return Column(
      children: [
        _buildAvatar(isPlayer1, accentColor, isDark),
        ...
        Expanded(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isDark ? (isPlayer1 ? const Color(0xFF131315) : const Color(0xFF1A1A1C)) : Colors.white,
              borderRadius: BorderRadius.circular(32),
              border: Border.all(
                color: isPlayer1 ? accentColor : (isDark ? const Color(0xFF2C2C2F) : Colors.black.withOpacity(0.05)),
                width: 2,
              ),
              boxShadow: isPlayer1 && isDark ? [
                BoxShadow(
            ...
            child: Center(
              child: Text(
                ...
                style: GoogleFonts.lexend(
                  fontSize: 72,
                  fontWeight: FontWeight.w900,
                  color: isPlayer1 ? accentColor : (isDark ? Colors.white : Colors.black),
                  letterSpacing: -2,
                  shadows: isPlayer1 && isDark ? [
                    Shadow(color: accentColor.withOpacity(0.5), blurRadius: 15)
                  ] : null,
                ),
              ),
            ),
  ...
  Widget _buildAvatar(bool isPlayer1, Color accentColor, bool isDark) {
    final playerAccent = isPlayer1 ? accentColor : (isDark ? const Color(0xFF2C2C2F) : const Color(0xFFE0E0E0));
    return Container(
      width: 80,
      height: 80,
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: playerAccent.withOpacity(0.3),
            blurRadius: 15,
          )
        ],
      ),
  ...
  Widget _buildScoreButton({
    ...
    final bgColor = isAdd 
        ? (isPlayer1 ? accentColor : (isDark ? const Color(0xFF1A1A1C) : Colors.white))
        : (isDark ? const Color(0xFF2C2C2F) : const Color(0xFFE6E8EA));
    final iconColor = isAdd 
        ? (isPlayer1 ? (isDark ? const Color(0xFF0D0D0F) : Colors.white) : (isDark ? Colors.white : Colors.black))
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: bgColor,
          borderRadius: BorderRadius.circular(isAdd ? 24 : 16),
          border: !isAdd ? null : (isPlayer1 ? null : Border.all(color: isDark ? const Color(0xFF2C2C2F) : Colors.black.withOpacity(0.05))),
          boxShadow: isAdd ? [
            BoxShadow(
              color: isPlayer1 ? accentColor.withOpacity(0.25) : Colors.black.withOpacity(0.1),
              blurRadius: 20,
              offset: const Offset(0, 4),
            )
          ] : null,
  ...
  Widget _buildFinishButton(BuildContext context, bool isDark) {
    ...
    final finishText = isDark ? const Color(0xFFFF6B6B) : const Color(0xFFB02500);
